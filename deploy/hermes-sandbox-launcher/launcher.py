...
def handle_connection(
    ...
    try:
        ...
        if action == "ping":
            ...
            exact_fields(payload, {"action", "project", "project_token"})
            project = authenticate_project(
                payload.get("project"),
                payload.get("project_token"),
                project_tokens,
            )
            ...
            exact_fields(
                payload,
                {"action", "run_id", "project", "project_token"},
            )
            project = authenticate_project(
                payload.get("project"),
                payload.get("project_token"),
            ...
            send_json(
                connection,
                {"ok": True, "cancelled": launcher.cancel(project, run_id)},
            )
        elif action == "run":
...
def main() -> int:
    validate_image_ids()
    project_tokens = load_project_tokens()
    launcher = Launcher()
    ...
    def worker(connection: socket.socket) -> None:
        try:
            handle_connection(connection, launcher, project_tokens)
        finally:
            slots.release()
