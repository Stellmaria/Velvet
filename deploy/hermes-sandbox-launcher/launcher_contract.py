...
def parse_env(path: Path) -> dict[str, str]:
    ...
    for raw in path.read_text(encoding="utf-8").splitlines():
        ...
        key = key.strip()
        if key in values:
            raise LauncherProtocolError(f"duplicate environment key: {key}")
...
def validate_image_ids() -> None:
    invalid = [
        project for project, image in IMAGES.items() if not _IMAGE_ID.fullmatch(image)
    ]
    if invalid:
        raise RuntimeError(
...
def load_project_tokens() -> dict[str, str]:
    tokens = {
        "velvet": os.environ.get("HERMES_SANDBOX_VELVET_TOKEN", "").strip(),
        "max": os.environ.get("HERMES_SANDBOX_MAX_TOKEN", "").strip(),
    }
    invalid = [project for project, token in tokens.items() if not _TOKEN.fullmatch(token)]
    if invalid:
        raise RuntimeError(
            "Sandbox project credentials are missing for: " + ", ".join(invalid)
        )
    if hmac.compare_digest(tokens["velvet"], tokens["max"]):
        raise RuntimeError("Sandbox project credentials must be distinct")
    return tokens
...
def authenticate_project(
    ...
    if not isinstance(project, str) or project not in _PROJECTS:
        raise LauncherProtocolError("invalid project credentials")
    if not isinstance(project_token, str) or not _TOKEN.fullmatch(project_token):
        raise LauncherProtocolError("invalid project credentials")
    expected = tokens.get(project, "")
    if not expected or not hmac.compare_digest(expected, project_token):
        raise LauncherProtocolError("invalid project credentials")
    return project
...
def create_codex_projection(project: str, run_id: str, route: str) -> Path:
    if project not in _PROJECTS or route not in _ROUTES or not _RUN_ID.fullmatch(run_id):
        raise LauncherProtocolError("invalid Codex projection request")
...
def cleanup_codex_projection(path: Path) -> None:
    if path.is_symlink():
