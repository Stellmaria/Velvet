...
class Launcher:
    ...
    def cleanup_stale(self) -> None:
        result = self._checked(
            ["docker", "ps", "-aq", "--filter", "label=hermes.sandbox=1"],
            timeout=30,
    ...
    def _stop_container(self, run_id: str, *, mark_cancelled: bool) -> bool:
    ...
    def cancel(self, run_id: str) -> bool:
        stopped = self._stop_container(run_id, mark_cancelled=True)
        if stopped:
            audit("run_cancel_requested", run_id=run_id)
        return stopped
    ...
    def run(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if cancelled_before_start:
                self._stop_container(run_id, mark_cancelled=True)
            ...
            try:
                ...
                timed_out = True
                self._stop_container(run_id, mark_cancelled=False)
                try:
