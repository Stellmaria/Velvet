...
assert_root_tree() {
  ...
  if [[ -n "$unsafe" ]]; then
    abort "$label contains non-root-owned or writable paths: $unsafe"
  fi
}
...
for fixed_dir in "$MIRROR_ROOT" "$RELEASE_ROOT" "$LOCK_DIR"; do
  if [[ -L "$fixed_dir" ]]; then
    abort "fixed directory must not be a symlink: $fixed_dir"
  fi
done
...
if [[ -e "$MIRROR_REPO" && ( -L "$MIRROR_REPO" || ! -d "$MIRROR_REPO/objects" ) ]]; then
  abort "release mirror path is not a regular bare Git repository"
fi
if [[ ! -d "$MIRROR_REPO/objects" ]]; then
  root_git init --bare "$MIRROR_REPO"
  root_git -C "$MIRROR_REPO" remote add origin "$REPOSITORY_URL"
else
  root_git -C "$MIRROR_REPO" remote set-url origin "$REPOSITORY_URL"
fi
assert_root_tree "$MIRROR_ROOT" "release mirror"

root_git -C "$MIRROR_REPO" fetch \
  --no-tags \
  --prune \
  --force \
  origin \
...
remote_main="$(root_git -C "$MIRROR_REPO" rev-parse refs/remotes/origin/main)"
if [[ "$remote_main" != "$TARGET_SHA" ]]; then
  abort "target is no longer current main: $TARGET_SHA != $remote_main"
fi
root_git -C "$MIRROR_REPO" cat-file -e "${TARGET_SHA}^{commit}"
...
if [[ -e "$release_dir" ]]; then
  ...
  assert_root_tree "$release_dir" "existing release worktree"
  if [[ "$(root_git -C "$release_dir" rev-parse HEAD)" != "$TARGET_SHA" ]]; then
    abort "existing release worktree has the wrong commit"
  fi
  root_git -C "$release_dir" diff --quiet
  root_git -C "$release_dir" diff --cached --quiet
  if [[ -n "$(root_git -C "$release_dir" ls-files --others --exclude-standard)" ]]; then
    abort "existing release worktree contains untracked files"
  fi
else
  root_git -C "$MIRROR_REPO" worktree prune
  root_git -C "$MIRROR_REPO" worktree add --detach "$release_dir" "$TARGET_SHA"
fi
