...
_REQUIRED_BOOLEAN_EVIDENCE = _MUTATION_SIGNALS + (
    "execution_started",
    "mutation_started",
)
...
def audit_ledger(ledger: Mapping[str, object]) -> tuple[bool, tuple[str, ...]]:
    """Validate trusted evidence and derive mutation without execution inflation."""
    ...
    if (
        type(ledger.get("head_changed")) is bool
        and ledger["head_changed"] != derived_head_changed
    ):
    ...
    if (
        type(ledger.get("mutation_started")) is bool
        and ledger["mutation_started"] != computed
    ):
...
def evaluate_review(request: ReviewRequest) -> ReviewDecision:
    ...
    verified: list[str] = []
    unverified: list[str] = list(request.rollout_only_checks)
    ...
    missing_checks = sorted(
        set(request.required_integration_checks) - successful_checks
    )
    ...
    if missing_checks:
        findings.append(
            "trusted integration checks missing or unsuccessful: "
            + ", ".join(missing_checks)
        )
    ...
    if ledger is None:
        ...
        required_ledger = {
            ...
            "baseline_head",
            "final_head",
        } | set(_REQUIRED_BOOLEAN_EVIDENCE)
        ...
        if request.github_mutation_observed and ledger_mutation is False:
            ...
            evidence_conflict = True
        if not audit_findings and (ledger_mutation is True or computed_mutation):
            verified.append("mutation_started is supported by trusted evidence")
        elif (
            not audit_findings
            and ledger.get("execution_started") is True
            and ledger_mutation is False
        ):
            verified.append("read-only execution started without mutation evidence")
