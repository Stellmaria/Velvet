...
class HermesIncidentClient:
    """Submit bounded, redacted incident packets to Hermes Runs API."""
    ...
    def submit(self, incident: HermesIncident) -> str:
        ...
        body = self._request_body(incident)
        ...
        try:
            ...
            raise RuntimeError(f"Hermes HTTP {error.code}: {details}") from error
        ...
        try:
            ...
            with self._lock:
                self._last_status = {
                    ...
                    "submitted_at": self._last_status.get("submitted_at"),
                    "fingerprint": fingerprint,
                }
            return
        with self._lock:
    ...
    def _request_body(self, incident: HermesIncident) -> dict[str, Any]:
        ...
        prompt = (
            ...
            "запрашивай секреты. Сначала классифицируй причину и предложи только "
            "действия из безопасного реестра: health-check, restart service, pause queue, "
            "switch configured fallback, run read-only diagnostics, prepare draft PR. "
            "Запрещены root/sudo, удаление данных, изменение production .env, прямой push "
            "в main и доступ к Docker socket.\n\n"
        ...
        return {
            ...
            "instructions": (
                "Ты аварийный диагност Velvet. Верни краткий диагноз, уровень риска, "
                "безопасное следующее действие и критерий эскалации в Codex."
            ),
        }
