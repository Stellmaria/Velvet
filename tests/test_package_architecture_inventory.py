...
class PackageArchitectureInventoryTests(unittest.TestCase):
    ...
    def test_inventory_covers_the_complete_current_package(self) -> None:
        ...
        self.assertEqual(656, self.inventory["production_module_count"])
        self.assertEqual(self.inventory["production_module_count"], len(modules))
        self.assertEqual(144094, self.inventory["production_loc"])
        ...
        self.assertEqual(21, len(self.inventory["installer_graph"]))
    ...
    def test_shared_private_and_duplicate_fingerprints_are_linked(self) -> None:
        ...
        self.assertEqual(655, shared["production_python_files"])
        self.assertEqual(3830, shared["function_count"])
        self.assertEqual(180, shared["private_contract_access_count"])
        self.assertEqual(0, shared["blocking_private_contract_access_count"])
        self.assertEqual(66, shared["exact_duplicate_group_count"])
        self.assertEqual(95, shared["normalized_duplicate_group_count"])
    ...
    def test_installer_graph_preserves_order_and_patch_evidence(self) -> None:
        ...
        self.assertEqual(list(range(1, 22)), [item["order"] for item in graph])
    ...
    def test_human_inventory_and_temporary_generator_contract(self) -> None:
        self.assertIn("Production modules: **656**", self.markdown)
        self.assertIn("Production LOC: **144094**", self.markdown)
