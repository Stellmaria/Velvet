...
class IsolatedPostgresSchemaTests(unittest.TestCase):
    def test_unittest_gets_private_schema(self) -> None:
        ...
        with (
            patch.dict(
                os.environ,
                {"TEST_DATABASE_URL": url, "TEST_DATABASE_SCHEMA": ""},
                clear=False,
            ),
    ...
    def test_production_process_does_not_switch_schema(self) -> None:
        ...
        with (
            ...
            patch.object(sys, "argv", ["python", "main.py"]),
        ):
        ...
        with (
            patch.dict(
                os.environ,
                {
                    ...
                    "TEST_DATABASE_SCHEMA": "bad-schema;drop",
                },
                clear=False,
            ),
...
class SupervisorTestEnvironmentTests(unittest.TestCase):
    @staticmethod
    def _repository(test_database_url: str | None) -> GitRepository:
    ...
    def test_inherited_test_database_is_removed(self) -> None:
        ...
        with (
            patch.dict(
                os.environ,
                {"TEST_DATABASE_URL": "postgresql://wrong/database"},
                clear=False,
            ),
            patch(
