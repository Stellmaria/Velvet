...
ALLOWED_DUPLICATE_COMMAND_ROUTES = {
    "characters", "prompt", "setprompt", "publish", "publishing",
    "publications", "checkpost", "refs", "ref", "refadd", "refdel",
    "refdone", "refcancel", "compare_ref", "compare_reference",
    "analytics", "analyticsmenu", "channelstats", "stats", "promptstats",
    "hashtagstats", "tagstats", "characterstats", "trackdiscussion",
    "discussionstats",
}
PANEL_COMMANDS = {
    "admin", "menu", "help", "system", "health", "version", "diag",
    "diagnostics", "analytics", "analyticsmenu", "channelstats", "stats",
    "promptstats", "characterstats", "backup", "quality", "auditarchive",
    "qwen_calibration", "qcalibration", "rework", "reworks",
    "quality_rework", "publish", "publishing", "publications", "characters",
    "storage", "storage_center", "supervisor", "status", "logs", "restart",
    "update", "rollback", "codex", "codex_status", "console",
    "supervisor_console", "supervisor_self", "workspace_setup",
    "workspace_guide", "workspace_setup_status", "myarchive",
    "archive_shortcuts", "taxonomy_manage", "structure_manage",
}
FORM_COMMANDS = {
    "create", "crete", "topic", "character", "category", "cat", "universe",
    "world", "series", "story", "stories", "storylist", "storyadd", "prompt",
    "setprompt", "refadd", "refs", "ref", "refdel", "compare_ref",
    "compare_reference", "analyze_set", "qwen_set", "aliasadd", "tagadd",
    "aliases", "tags", "aliasdel", "tagdel", "tagstats", "hashtagstats",
    "trackdiscussion", "discussionstats",
}
CONTEXT_COMMANDS = {
    "save", "save18", "checkpost", "importchannel", "importdiscussion",
    "watermark",
}
DIRECT_COMMANDS = {
    "cancel", "savecancel", "refdone", "refcancel", "aliasreindex",
    "tagreindex", "test_error_alert", "diag_export", "storage_migrate",
    "storage_find", "storage_download", "wm_file", "wm_storage", "wm_download",
    "workspace_grant", "workspace_revoke", "workspace_module", "workspace_bind",
    "workspace_unbind", "workspace_bind_channel", "workspace_delete",
    "rp_help", "rp_on", "rp_off", "rp", "rp_prompt", "rp_status", "rp_reset",
}
...
def _literal_strings(call: ast.Call) -> list[str]:
    return [argument.value for argument in call.args
        ...
        for node in ast.walk(tree):
                ...
                for decorator in node.decorator_list:
                    ...
                    for argument in decorator.args:
                        if (isinstance(argument, ast.Call)
                                and _call_name(argument.func).endswith("Command")):
                            for command in _literal_strings(argument):
                                routes[command].append(f"{path}:{node.lineno}:{node.name}")
                continue
            if not isinstance(node, ast.Call):
                continue
            if not _call_name(node.func).endswith("router.message.register") or not node.args:
                continue
            ...
            for argument in node.args[1:]:
                if (isinstance(argument, ast.Call)
                        and _call_name(argument.func).endswith("Command")):
                    for command in _literal_strings(argument):
                        routes[command].append(f"{path}:{node.lineno}:{handler_name}")
    return routes
...
class ProjectIntegrityTests(unittest.TestCase):
    ...
    def test_callback_prefixes_are_unique(self) -> None:
        duplicates = {prefix: owners for prefix, owners in _callback_prefixes().items()
                      if len(owners) > 1}
        self.assertEqual(duplicates, {})

    def test_every_real_command_has_an_explicit_ui_or_reserve_route(self) -> None:
        actual = set(_command_routes())
        covered = (PANEL_COMMANDS | FORM_COMMANDS | CONTEXT_COMMANDS
                   | DIRECT_COMMANDS | PUBLIC_COMMANDS)
        self.assertEqual(actual, covered)

    def test_only_filter_disambiguated_commands_have_multiple_handlers(self) -> None:
        duplicate_routes = {command for command, routes in _command_routes().items()
                            if len(routes) > 1}
    ...
    def test_migration_numbers_are_unambiguous_except_legacy_pair(self) -> None:
        ...
        duplicates = {number: names for number, names in by_number.items()
                      if len(names) > 1}
    ...
    def test_owner_callbacks_are_typed_and_not_split_across_fix_router(self) -> None:
        ...
        for path in (Path("velvet_bot/owner_menu.py"), Path(
