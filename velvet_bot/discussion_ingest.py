...
from velvet_bot.domains.discussions import DiscussionIngestResult
...
async def ingest_live_discussion_message(
    ...
    result: DiscussionIngestResult = await build_discussion_ingest_service(
        database
    ).ingest(discussion_event_from_message(message))
