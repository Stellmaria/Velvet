...
def install_meow_runtime_dispatcher() -> None:
    """Replace fixed Kie slots with one database-driven provider dispatcher."""
    ...
    def build_worker_manager_with_meow_runtime(*args: Any, **kwargs: Any):
        ...
        if database is None or bot is None:
            raise RuntimeError("Динамический диспетчер Мяу требует bot и database.")
        usage_service = kwargs.get("ai_usage_service") or build_ai_usage_service(
            database=database
        )
        if kie_settings.api_key is None:
            raise RuntimeError("Мяу включён без KIE_API_KEY.")
        ...
        dispatcher = MeowGenerationDispatcher(
            bot=bot,
            database=database,
            client=client,
    ...
    workers.build_worker_manager = build_worker_manager_with_meow_runtime
    bootstrap.build_worker_manager = build_worker_manager_with_meow_runtime
    _INSTALLED = True


__all__ = ("install_meow_runtime_dispatcher",)
