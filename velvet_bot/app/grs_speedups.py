...
# A process restart or a short TTL automatically discards stale entries.
...
def _fast_client_init(self: KieClient, *args: Any, **kwargs: Any) -> None:
    ...
    kwargs["poll_interval_seconds"] = min(configured, _MAX_POLL_INTERVAL_SECONDS)
...
async def _fast_start_progress(
    ...
    *,
    ...
    chat_id = self._optional_int(task.payload.get("chat_id")) if hasattr(self, "_optional_int") else None
    if chat_id is None:
        # Friendly worker uses the module helper rather than an instance method.
        from velvet_bot.domains.media_generation.worker import _optional_int
    ...
    *,
    ...
    async def prepare(index: int, reference: object) -> str:
        ...
        async with semaphore:
            # Recheck after waiting so equal references from simultaneous tasks do not
            # needlessly survive in the slow path whenever an earlier upload finished.
    ...
    urls = await asyncio.gather(
        *(prepare(index, reference) for index, reference in enumerate(request.references, start=1))
    )
...
def install_grs_speedups() -> None:
    ...
    FriendlyKieGenerationWorker._start_progress = _fast_start_progress  # type: ignore[method-assign]
    FriendlyKieGenerationWorker._friendly_progress_text = _fast_friendly_text  # type: ignore[method-assign]
    BaseKieGenerationWorker._upload_references = _fast_upload_references  # type: ignore[method-assign]
    _INSTALLED = True
