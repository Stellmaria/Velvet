    ...
    *,
    ...
    if can_submit:
        rows.append(
            [
                InlineKeyboardButton(
                    text=f"Запустить · {format_auf_units(quoted_units)}",
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text="Пересчитать баланс и цену",
                    callback_data=video_router.legacy._callback(
                        "review",
                        workspace_id=workspace_id,
                    ),
...
async def _show_video_auf_review(
    ...
    *,
    ...
    enough = global_owner or wallet.available_units >= quote.quoted_units
    active = global_owner or wallet.status is AufWalletStatus.ACTIVE
    can_submit = enough and active
    if global_owner:
        wallet_lines = [
            f"Учётная цена: <b>{format_auf_units(quote.quoted_units)}</b>",
            "Списание Стэл: <b>0 Ауф</b>",
        ]
    elif wallet.status is AufWalletStatus.FROZEN:
        wallet_lines = [
        ...
        wallet_lines = [
            ...
            "Останется: "
            f"<b>{format_auf_units(wallet.available_units - quote.quoted_units)}</b>",
        ]
    else:
        wallet_lines = [
            f"Цена: <b>{format_auf_units(quote.quoted_units)}</b>",
            f"Доступно: <b>{format_auf_units(wallet.available_units)}</b>",
            "Не хватает: "
            f"<b>{format_auf_units(quote.quoted_units - wallet.available_units)}</b>",
        ]
...
async def _submit_video_with_auf(
    ...
    *,
    ...
    alias = video_router._MODEL_ALIASES[model]
...
def _task_line(row) -> str:
    ...
    if quoted_units > 0:
        ...
        charge = "без операции Ауф"
    params = [value for value in (resolution, f"{duration} сек" if duration else "") if value]
...
async def _render_user_tasks(
    ...
    *,
    ...
    text = (
        ...
        + ("\n\n".join(_task_line(row) for row in page) if page else "• задач пока нет")
    )
    await video_router.legacy._edit_or_answer(
        callback,
        text=text,
