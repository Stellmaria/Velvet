...
async def handle_start(message: Message, bot_username: str) -> None:
    ...
    await message.answer(
        ...
        "Добавить альбом референсов в личке:\n"
        "<code>/refadd Аид</code> → отправить фото → <code>/refdone</code>\n\n"
        "Добавить один референс из любого чата: ответьте на фото и отправьте:\n"
        f"<code>@{safe_username} refadd Аид</code>\n\n"
        "Показать референсы: <code>/refs Аид</code>\n"
        "Удалить выбранный референс: кнопка <b>🗑 Удалить референс</b>\n"
        "Удалить по номеру: <code>/refdel Аид 2</code>\n\n"
