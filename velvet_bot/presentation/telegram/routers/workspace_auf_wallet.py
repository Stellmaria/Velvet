...
from velvet_bot.domains.auf_runtime import AUF_WORKSPACE_ACTION
...
async def handle_auf_wallet_action(
    ...
    try:
        if action == "wallet_grant":
            ...
            return
    except (AufWalletAccessError, ValueError, RuntimeError) as error:
        await callback.answer(str(error), show_alert=True)
        return
...
# Existing callback wiring may still import the old handler name.
