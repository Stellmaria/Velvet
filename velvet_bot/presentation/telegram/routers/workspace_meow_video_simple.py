...
async def handle_meow_video_entry(
    callback: CallbackQuery, callback_data: MeowCallback, state: FSMContext,
    ...
    await state.update_data(
        meow_video_session_id=uuid4().hex,
        meow_video_workspace_id=workspace_id,
        meow_video_reference=None,
        meow_video_last_reference=None,
        meow_video_reference_target="first",
        meow_video_text_target="prompt",
        meow_video_prompt="",
        ...
        meow_video_wan_mode="first",
        meow_video_cost_change=None,
    )
    await state.set_state(MeowVideoForm.choosing_reference)
...
async def handle_meow_video_reference_message(
    message: Message, state: FSMContext, access_policy: AccessPolicy,
    kie_settings: KieSettings,
) -> None:
    data = await state.get_data()
    if str(data.get("meow_video_reference_target") or "first") != "last":
        await legacy.handle_meow_video_reference_message(message, state, access_policy, kie_settings)
        return
    ...
    await state.update_data(meow_video_last_reference=reference.to_payload(), meow_video_reference_target="first")
...
async def handle_meow_video_prompt(
    ...
    if not kie_settings.enabled:
        ...
        return
    data = await state.get_data()
    target = str(data.get("meow_video_text_target") or "prompt")
    workspace_id = legacy._optional_int(data.get("meow_video_workspace_id")) or 0
    if target == "duration":
        ...
        await state.update_data(meow_video_duration=duration, meow_video_text_target="prompt")
        new_data = await state.get_data()
        await _store_cost_change(
            state,
            old_estimate=old_estimate,
    ...
    if legacy._reference_from_data(data.get("meow_video_reference")) is None:
        ...
        return
    had_prompt = bool(str(data.get("meow_video_prompt") or "").strip())
    await state.update_data(meow_video_prompt=prompt, meow_video_text_target="prompt")
    await state.set_state(MeowVideoForm.choosing_settings)
    if had_prompt:
        await _answer_settings(message, state=state, workspace_id=workspace_id, kie_settings=kie_settings)
    else:
...
async def handle_meow_video_action(
    callback: CallbackQuery, callback_data: MeowVideoCallback, state: FSMContext,
    ...
    if action not in custom_actions:
        if action in {"change_photo", "sources", "upload", "database", "select_reference"}:
            await state.update_data(meow_video_reference_target="first")
    ...
    if action == "resolution":
        ...
        old_estimate = _estimate_from_data(data, kie_settings=kie_settings)
        await state.update_data(meow_video_resolution=resolution)
        new_data = await state.get_data()
    ...
    if action == "audio":
        ...
        old_estimate = _estimate_from_data(data, kie_settings=kie_settings)
        await state.update_data(meow_video_generate_audio=new_audio)
        new_data = await state.get_data()
    ...
    if action == "duration_input":
        ...
        await state.set_state(MeowVideoForm.waiting_prompt)
        await legacy._edit_or_answer(
            callback,
    ...
    if action == "wan_mode":
        ...
        old_estimate = _estimate_from_data(data, kie_settings=kie_settings)
        await state.update_data(meow_video_wan_mode=callback_data.value)
        new_data = await state.get_data()
    ...
    if action == "last_frame":
        ...
        if _validated_model(data) != "wan" or _validated_wan_mode(data) != "first_last":
            ...
            return
        await state.update_data(meow_video_reference_target="last")
        await state.set_state(MeowVideoForm.waiting_reference)
        await legacy._edit_or_answer(
            callback,
    ...
    if action == "change_prompt":
        await state.update_data(meow_video_text_target="prompt")
        await state.set_state(MeowVideoForm.waiting_prompt)
        await legacy._edit_or_answer(
            callback,
    ...
    if action == "template_apply":
        ...
        return
    if action == "settings":
        await state.update_data(meow_video_reference_target="first", meow_video_text_target="prompt")
        await _show_settings(callback, state=state, workspace_id=workspace_id, kie_settings=kie_settings)
        return
    if action == "review":
...
async def _show_settings(
    ...
    has_last_frame = legacy._reference_from_data(data.get("meow_video_last_reference")) is not None
    estimated_usd, estimated_rub = _estimate_from_data(data, kie_settings=kie_settings)
    cost_change = data.get("meow_video_cost_change")
    await state.set_state(MeowVideoForm.choosing_settings)
    await legacy._edit_or_answer(
        callback,
        text=_settings_text(
...
async def _show_review(
    ...
    last_reference = legacy._reference_from_data(data.get("meow_video_last_reference"))
...
async def _submit_video(
    ...
    await legacy._edit_or_answer(callback, text="\n".join(details), reply_markup=build_meow_root_keyboard(workspace_id=workspace_id, enabled=True))
...
def _estimate_from_data(data: Mapping[str, object], *, kie_settings: KieSettings) -> tuple[Decimal, Decimal]:
    ...
    request = _build_request(
        reference=reference,
        last_reference=legacy._reference_from_data(data.get("meow_video_last_reference")),
        prompt=str(data.get("meow_video_prompt") or "motion") or "motion",
        model=model,
        resolution=_validated_resolution(data, model=model),
        duration=_validated_duration(data, model=model),
        generate_audio=_validated_audio(data, model=model),
...
async def _store_cost_change(
    ...
    await state.update_data(meow_video_cost_change={
...
async def _apply_model_defaults(
    ...
    if model == "grok15":
        ...
        await state.update_data(meow_video_model="grok", meow_video_resolution="480p", meow_video_duration=6, meow_video_generate_audio=False, meow_video_wan_mode="first")
...
async def _apply_template(state: FSMContext, *, model: str, template: Mapping[str, object]) -> None:
    ...
    await state.update_data(
        ...
        meow_video_resolution=resolution,
        meow_video_duration=duration,
        meow_video_generate_audio=bool(template.get("generate_audio")) if model == "seedance" else False,
        meow_video_wan_mode=wan_mode if model == "wan" and wan_mode in _WAN_MODES else "first",
    )
...
def _validated_model(data: Mapping[str, object] | object) -> str:
    model = str(data.get("meow_video_model") or "grok") if isinstance(data, Mapping) else "grok"
...
def _validated_audio(data: Mapping[str, object] | object, *, model: str) -> bool:
    return bool(model == "seedance" and isinstance(data, Mapping) and data.get("meow_video_generate_audio") is True)


def _validated_wan_mode(data: Mapping[str, object] | object) -> str:
    value = str(data.get("meow_video_wan_mode") or "first") if isinstance(data, Mapping) else "first"
