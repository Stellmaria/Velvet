...
from velvet_bot.domains.roleplay.service import (
    RoleplayInactiveError, RoleplayService, RoleplayUnavailableError,
)
...
class ActiveRoleplayFilter(BaseFilter):
    async def __call__(self, message: Message, **data: object) -> bool:
        if message.chat.type != ChatType.PRIVATE or message.from_user is None or not message.text:
            return False
...
def _split_text(text: str, *, limit: int = _TELEGRAM_CHUNK_LIMIT) -> tuple[str, ...]:
...
async def _generate_reply(message: Message, *, roleplay_service: RoleplayService,
                          text: str, activate: bool) -> None:
    ...
    try:
        ...
        reply = await roleplay_service.reply(
            chat_id=actor[0], user_id=actor[1], text=text, activate=activate)
    except (RoleplayClientError, RoleplayInactiveError,
            RoleplayUnavailableError, ValueError) as error:
        await _answer_plain(message, str(error))
        return
...
async def handle_roleplay_help(message: Message) -> None:
    await message.answer(
        ...
        "<code>/rp_status</code> — показать модель, режим и объём памяти.\n"
...
async def handle_roleplay_on(message: Message,
                             roleplay_service: RoleplayService) -> None:
    actor = _actor_ids(message)
    if actor is None:
        return
    try:
...
async def handle_roleplay_off(message: Message,
                              roleplay_service: RoleplayService) -> None:
    actor = _actor_ids(message)
    if actor is None:
        return
...
async def handle_roleplay_reset(message: Message,
                                roleplay_service: RoleplayService) -> None:
    actor = _actor_ids(message)
    if actor is None:
        return
    await roleplay_service.reset(chat_id=actor[0], user_id=actor[1])
    await message.answer("История РЛ и краткая память очищены. Постоянный канон сохранён.")


@router.message(Command("rp_prompt"))
async def handle_roleplay_prompt(message: Message,
                                 roleplay_service: RoleplayService) -> None:
    actor = _actor_ids(message)
    if actor is None:
        return
    ...
    if not canon:
        session = await roleplay_service.get_session(chat_id=actor[0], user_id=actor[1])
        ...
        return
    try:
        await roleplay_service.set_canon(chat_id=actor[0], user_id=actor[1], canon=canon)
...
async def handle_roleplay_status(message: Message,
                                 roleplay_service: RoleplayService) -> None:
    actor = _actor_ids(message)
    if actor is None:
        return
    ...
    await message.answer(
        ...
        f"Провайдер: <code>{escape(roleplay_service.provider_label)}</code>\n"
        f"Модель: <code>{escape(roleplay_service.model_label)}</code>\n"
...
async def handle_roleplay_command(message: Message,
                                  roleplay_service: RoleplayService) -> None:
    text = _command_argument(message)
    if not text:
        await message.answer("Добавьте реплику после команды: <code>/rp текст</code>.")
        return
    await _generate_reply(message, roleplay_service=roleplay_service,
                          text=text, activate=True)


@router.message(ActiveRoleplayFilter())
async def handle_active_roleplay_message(message: Message,
                                         roleplay_service: RoleplayService) -> None:
    await _generate_reply(message, roleplay_service=roleplay_service,
                          text=message.text or "", activate=False)
