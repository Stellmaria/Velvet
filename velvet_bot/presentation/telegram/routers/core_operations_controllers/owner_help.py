...
OWNER_COMMAND_GROUPS: tuple[tuple[str, tuple[tuple[str, str], ...]], ...] = (
    ("Навигация и публичный архив", (
        ("start", "открыть главное меню"), ("help", "показать полный справочник владельца"),
        ("menu", "открыть центр управления"), ("admin", "алиас центра управления"),
        ("archive", "открыть выбранный публичный архив персонажей"),
        ("gallery", "алиас публичного архива"),
        ("cancel", "отменить текущую форму или ожидание файла"),
    )),
    ("Личные пространства", (
        ...
        ("myarchive", "показать быстрые команды активного личного архива"),
        ("archive_shortcuts", "алиас быстрых команд личного архива"),
        ("taxonomy_manage", "изменить или удалить личную структуру архива"),
        ("structure_manage", "алиас управления структурой архива"),
    )),
    ("Персонажи, категории и истории", (
        ("characters", "открыть каталог персонажей"), ("create", "создать персонажа"),
        ...
        ("world", "алиас изменения вселенной"), ("series", "алиас изменения вселенной"),
        ("story", "назначить персонажу историю"),
        ("stories", "показать истории вселенной"), ("storylist", "алиас списка историй"),
    ...
    ("Медиа, референсы и алиасы", (
        ("save", "сохранить медиа персонажа"), ("save18", "сохранить медиа со спойлером"),
        ("savecancel", "отменить ожидание файла для сохранения"),
        ("watermark", "открыть обработку водяного знака"),
        ("refadd", "начать загрузку референсов"), ("refs", "показать референсы персонажа"),
        ("ref", "алиас просмотра референсов"), ("refdel", "удалить референс по номеру"),
        ...
        ("aliasadd", "добавить алиас персонажа"), ("tagadd", "алиас добавления тега"),
        ("aliases", "показать алиасы персонажа"), ("tags", "алиас просмотра тегов"),
        ("aliasdel", "удалить алиас персонажа"), ("tagdel", "алиас удаления тега"),
    ...
    ("Аналитика, обсуждения и импорт", (
        ...
        ("channelstats", "показать статистику канала"), ("stats", "алиас статистики канала"),
    ...
    ("Публикации", (
        ("publish", "открыть центр публикаций"), ("publishing", "алиас центра публикаций"),
    ...
    ("Velvet AI и контроль качества", (
        ...
        ("rework", "открыть очередь доработки"), ("reworks", "алиас очереди доработки"),
    ...
    ("Система, диагностика и резервные копии", (
        ("system", "открыть системный центр"), ("health", "алиас проверки состояния системы"),
    ...
    ("Supervisor, Git и Codex", (
        ("supervisor", "открыть Supervisor"), ("status", "показать состояние Supervisor"),
        ("logs", "показать логи бота"), ("restart", "перезапустить процесс бота"),
        ("update", "обновить проект из main"), ("rollback", "откатить последнее обновление"),
        ("codex", "открыть задачи Codex"), ("codex_status", "показать статус задачи Codex"),
...
OWNER_HELP_COMMANDS = frozenset(
    command for _, entries in OWNER_COMMAND_GROUPS for command, _ in entries)


def build_owner_help_pages() -> tuple[str, ...]:
    intro = ("<b>📋 Все команды Velvet</b>\n\n"
    ...
    for title, entries in OWNER_COMMAND_GROUPS:
        ...
        block_lines.extend(f"<code>/{command}</code> — {description}"
...
__all__ = ("OWNER_COMMAND_GROUPS", "OWNER_HELP_COMMANDS", "build_owner_help_pages",
           "handle_owner_help", "router")
