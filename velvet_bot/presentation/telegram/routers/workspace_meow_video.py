...
class MeowVideoCallback(CallbackData, prefix="meowv"):
    action: str
    workspace_id: int = 0
    value: str = ""
    item_id: int = 0
...
async def handle_meow_video_entry(
    ...
    await state.update_data(
        ...
        meow_video_duration=6,
        meow_video_aspect_ratio="9:16",
        meow_video_mode="normal",
    )
...
async def handle_meow_video_action(
    ...
    if action == "select_reference":
        ...
        await state.update_data(meow_video_reference=reference.to_payload())
...
async def handle_meow_video_reference_message(
    ...
    workspace_id = _optional_int(data.get("meow_video_workspace_id")) or 0
    await state.update_data(meow_video_reference=reference.to_payload())
...
async def handle_meow_video_prompt(
    ...
    if len(prompt) > _MAX_PROMPT_LENGTH:
        ...
        return
    data = await state.get_data()
    if _reference_from_data(data.get("meow_video_reference")) is None:
        await state.clear()
    ...
    await message.answer(
        _settings_text(
            ...
            duration=_optional_int(data.get("meow_video_duration")) or 6,
            aspect_ratio=str(data.get("meow_video_aspect_ratio") or "9:16"),
            mode=str(data.get("meow_video_mode") or "normal"),
        ),
...
async def _show_review(
    ...
    *,
    ...
    data = await state.get_data()
    reference = _reference_from_data(data.get("meow_video_reference"))
...
async def _update_setting(state: FSMContext, *, action: str, value: str) -> None:
    if action == "resolution" and value in _VIDEO_RESOLUTIONS:
        await state.update_data(meow_video_resolution=value)
        ...
        await state.update_data(meow_video_aspect_ratio=value)
    elif action == "mode" and value in _VIDEO_MODES:
        await state.update_data(meow_video_mode=value)
