...
async def handle_auf_runtime_action(
    ...
    if callback_data.action != "visibility_toggle":
        ...
        return
    current = await auf_runtime_service.module_is_visible(
        workspace_id=workspace_id,
        actor_user_id=callback.from_user.id,
    )
    try:
        visible = await auf_runtime_service.set_module_visible(
            workspace_id=workspace_id,
            actor_user_id=callback.from_user.id,
...
async def handle_auf_runtime_callback(
    ...
    if action == "max_limits":
        try:
            for provider in AufProvider:
                await auf_runtime_service.set_provider_limit(
                    actor_user_id=user_id,
                    provider=provider,
                    limit=100,
                )
    ...
    if action == "confirm":
        try:
            await auf_runtime_service.confirm_runtime_settings(
                actor_user_id=user_id
            )
        except AufRuntimeAccessError as error:
            await callback.answer(str(error), show_alert=True)
    ...
    if action == "workspace_limit":
        try:
            await auf_runtime_service.set_workspace_limit(
                workspace_id=workspace_id,
                actor_user_id=user_id,
...
async def handle_auf_runtime_limit_input(
    ...
    workspace_id = int(
        ...
        or data.get("auf_runtime_workspace_id")
        or 0
    )
    target = str(
        data.get("auf_runtime_limit_target")
        or data.get("auf_runtime_limit_target")
        or ""
    )
    try:
        limit = int((message.text or "").strip())
        if target == "workspace":
            settings = await auf_runtime_service.set_workspace_limit(
                workspace_id=workspace_id,
            ...
            settings = await auf_runtime_service.set_provider_limit(
                actor_user_id=message.from_user.id,
                provider=provider,
                limit=limit,
            )
