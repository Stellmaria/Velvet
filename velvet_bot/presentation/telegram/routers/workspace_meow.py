...
def build_meow_mode_keyboard(*, workspace_id: int) -> InlineKeyboardMarkup:
...
async def _session_data(
    ...
    return (
        _optional_int(data.get("meow_workspace_id")) or 0,
...
async def _show_review(
    ...
    *,
    ...
    if message:
        ...
        return
    await state.set_state(MeowForm.reviewing_request)
...
async def handle_meow_action(
    ...
    if action == "create":
        await state.clear()
        await state.update_data(meow_workspace_id=workspace_id)
        await _edit_or_answer(
            callback,
            text=(
                "<b>Мяу · Создать</b>\n\n"
            ...
            reply_markup=build_meow_mode_keyboard(workspace_id=workspace_id),
        )
        return
    if action == "animate":
        await callback.answer(
    ...
    if action == "mode":
        ...
        await state.clear()
        await state.update_data(
            meow_workspace_id=workspace_id,
        ...
        if input_mode is KieInputMode.TEXT:
            ...
            return
        await state.set_state(MeowForm.collecting_references)
        await _edit_or_answer(
            callback,
...
async def handle_meow_reference_message(
    message: Message,
    state: FSMContext,
    access_policy: AccessPolicy,
    kie_settings: KieSettings,
...
async def handle_meow_reference_text(
    message: Message,
    state: FSMContext,
    access_policy: AccessPolicy,
    kie_settings: KieSettings,
