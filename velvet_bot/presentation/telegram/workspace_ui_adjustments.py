...
def _media_card_keyboard(*args, **kwargs) -> InlineKeyboardMarkup:
    ...
    for row in keyboard.inline_keyboard:
        ...
        for button in row:
            ...
            elif button.text == "🛠 Отправить на доработку":
                normalized.append(
                    InlineKeyboardButton(
                        text="🛠 Доработать / завершить",
                        callback_data=button.callback_data,
                    )
                )
            else:
...
async def _show_media_help(
    ...
    *,
    ...
    text = (
        ...
        "<b>Доработать / завершить</b> — первое нажатие создаёт заявку только внутри "
        "текущего пространства и скрывает материал из его публичной выдачи. Повторное "
        "нажатие завершает заявку. После этого материал остаётся скрытым, пока владелец "
        "отдельно не нажмёт «Вернуть в публичный».\n\n"
        "<b>Скрыть из публичного / Вернуть в публичный</b> — меняет видимость "
        "конкретной работы. Вернуть материал нельзя, пока его заявка на доработку "
        "в этом пространстве активна.\n\n"
