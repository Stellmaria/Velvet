...
async def _show_media_help(
    ...
    *,
    ...
    text = (
        ...
        "Оригинал не заменяется до явного подтверждения. Если отдельное назначение "
        "«Watermark-копии» не подключено, готовый PNG возвращается в текущий чат.\n\n"
        ...
        "версию бот выдаёт читателям. Для watermark-версии нужен настроенный логотип "
        "и подтверждённая копия материала; отдельное хранилище остаётся необязательным.\n\n"
