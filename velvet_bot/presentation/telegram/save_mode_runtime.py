...
def register_save_mode_handlers(parent_router: Router) -> None:
    router_id = id(parent_router)
    if router_id in _REGISTERED_ROUTER_IDS:
        return
    _REGISTERED_ROUTER_IDS.add(router_id)
    parent_router.message.register(handle_save_mode_command, SaveModeCommandFilter())
