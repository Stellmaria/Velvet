        ...
        try:
            ...
            if not urls:
                raise ProviderResultPending(
                    "Provider reported success without stable result URLs."
                )
            await self._repository.record_provider_success(
                task_id=job.task_id,
                provider=job.provider,
            ...
            await self._finish_cancelled(job.task_id)
            raise
        except ProviderResultTerminal as error:
            ...
            logger.warning(
                "media_delivery_resolution task=%s provider=%s "
                "outcome=failed code=%s",
                job.task_id,
                job.provider,
                error.code,
            )
            return False
        except MediaDeliveryProviderError as error:
            terminal = not error.retryable or job.attempt_count >= self._max_attempts
            await self._repository.finish_resolution(
                task_id=job.task_id,
                worker_id=self._worker_id,
                error=error,
                retry_delay_seconds=(
                    None if terminal else self._retry_delay(job.attempt_count)
                ),
                terminal=terminal,
            )
            logger.warning(
                "media_delivery_resolution task=%s provider=%s outcome=%s code=%s",
                job.task_id,
                job.provider,
            ...
            logger.exception(
                "Media delivery resolution persistence failed task=%s provider=%s",
                job.task_id,
                job.provider,
            )
            raise
    ...
    async def _finish_cancelled(self, task_id: UUID) -> None:
        try:
            await self._repository.finish_resolution(
                task_id=task_id,
                worker_id=self._worker_id,
                error=ProviderResultPending("Result resolution worker cancelled."),
                retry_delay_seconds=5,
            )
        except MediaDeliveryRepositoryError:
            logger.exception(
                "Could not release cancelled media result resolution task=%s",
                task_id,
            )
