...
class AufWalletRepository:
    """Persist Auf wallets while retaining deployed legacy SQL table names."""

    def __init__(self, database: Database) -> None:
        self._database = database

    async def economy_settings(self) -> AufEconomySettings:
    ...
    async def wallet(self, workspace_id: int) -> AufWallet:
        async with self._database.acquire() as connection:
...
def _settings_from_row(row: Mapping[str, Any]) -> AufEconomySettings:
    return AufEconomySettings(
...
def _wallet_from_row(row: Mapping[str, Any]) -> AufWallet:
    return AufWallet(
        workspace_id=int(row["workspace_id"]),
        available_units=int(row["available_units"]),
        reserved_units=int(row["reserved_units"]),
        status=AufWalletStatus(str(row["status"])),
        created_at=row["created_at"],
        updated_at=row["updated_at"],
    )


def _entry_from_row(row: Mapping[str, Any]) -> AufWalletEntry:
    metadata = row["metadata"]
    if isinstance(metadata, str):
        metadata = json.loads(metadata)
    return AufWalletEntry(
        id=int(row["id"]),
        workspace_id=int(row["workspace_id"]),
        operation_type=AufWalletOperation(str(row["operation_type"])),
