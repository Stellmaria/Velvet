...
class MeowInvoiceError(RuntimeError):
    pass


class MeowPurchaseRepository:
    ...
    async def invoice_by_code(self, public_code: str) -> MeowPurchaseInvoice | None:
        async with self._database.acquire() as connection:
            row = await connection.fetchrow(
        ...
        *,
    ...
    ) -> tuple[MeowPurchaseInvoice, MeowWallet]:
        ...
        async with self._database.acquire() as connection:
            async with connection.transaction():
                ...
                if row is None:
                    raise MeowInvoiceError("Счёт Ауф не найден.")
                ...
                if invoice.status is MeowInvoiceStatus.PAID:
                    return invoice, _wallet_from_row(wallet_row)
                if invoice.status is not MeowInvoiceStatus.CREATED:
                    raise MeowInvoiceError(
                ...
                if invoice.expires_at <= datetime.now(timezone.utc):
                    ...
                    raise MeowInvoiceError("Срок действия счёта Ауф истёк.")
        ...
        *,
        ...
        if row is None:
            ...
            if existing is None or existing.workspace_id != int(workspace_id):
                raise MeowInvoiceError("Счёт Ауф не найден.")
            if existing.status is MeowInvoiceStatus.CANCELLED:
                return existing
            raise MeowInvoiceError(
    ...
    async def claim_reconciliation_notice(
        self,
        issues: tuple[MeowReconciliationIssue, ...],
    ) -> bool:
        payload = [
...
class MeowPurchaseService:
    def __init__(
        self,
        repository: MeowPurchaseRepository,
...
def _invoice_from_row(row: Mapping[str, Any]) -> MeowPurchaseInvoice:
    return MeowPurchaseInvoice(
        id=row["id"],
        ...
        status=MeowInvoiceStatus(str(row["status"])),
