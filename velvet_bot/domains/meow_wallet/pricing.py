...
class MeowPriceNotConfigured(ValueError):
    pass


class MeowPricingRepository:
    def __init__(self, database: Database) -> None:
        self._database = database

    async def quote(self, payload: Mapping[str, object]) -> MeowPriceQuote:
        async with self._database.acquire() as connection:
            return await quote_meow_payload(connection, payload)


async def quote_meow_payload(
    connection: Any,
    payload: Mapping[str, object],
) -> MeowPriceQuote:
    request_value = payload.get("request")
    if not isinstance(request_value, Mapping):
        raise MeowPriceNotConfigured("В AI-задаче отсутствуют параметры генерации.")

    model_alias = str(request_value.get("model") or "").strip()
    resolution = str(request_value.get("resolution") or "").strip()
    if not model_alias:
        raise MeowPriceNotConfigured("В AI-задаче не указана модель генерации.")
