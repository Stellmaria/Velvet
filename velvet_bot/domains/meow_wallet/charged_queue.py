...
from .models import MeowInsufficientBalance, MeowWalletFrozen
from .pricing import MeowPriceQuote, quote_meow_payload


class MeowChargedTaskQueueService(AITaskQueueService):
    """Atomically reserve Auf and enqueue paid Meow generation tasks."""
    ...
    async def enqueue(self, request: AITaskRequest) -> AITaskEnqueueResult:
        ...
        async with self._database.acquire() as connection:
            async with connection.transaction():
                quote = await quote_meow_payload(connection, request.payload)
...
def build_meow_charged_task_queue_service(
    *,
    database: Database,
) -> MeowChargedTaskQueueService:
    return MeowChargedTaskQueueService(database)
...
async def _reserve_charge(
    ...
    *,
    ...
    if str(wallet["status"]) != "active":
        raise MeowWalletFrozen("Кошелёк Ауф этого пространства заморожен.")
