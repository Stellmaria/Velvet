    ...
    try:
        ...
        if not await _module_enabled(
            ...
            raise WorkspaceAccessError(
                "Модуль референсов выключен или не разрешён Стэл."
            )
        if require_qwen:
            if not await _module_enabled(
                database,
                ...
                raise WorkspaceAccessError(
                    "Модуль Qwen выключен или не разрешён Стэл."
                )
            settings = await workspace_service._repository.get_settings(workspace.id)
            if settings is not None and not settings.qwen_enabled:
