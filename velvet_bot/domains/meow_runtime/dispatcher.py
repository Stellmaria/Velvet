...
class MeowGenerationDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        self._bot = bot
        self._runtime = MeowRuntimeRepository(database)
        ...
        self._queues = {
            provider: ProviderMeowTaskQueueService(
                database=database,
                provider=provider,
        ...
        self._active: dict[MeowProvider, set[asyncio.Task[int]]] = {
            provider: set() for provider in MeowProvider
        }
        self._sequence = 0
        self._settings = None
    ...
    def _spawn(self, provider: MeowProvider) -> None:
        self._sequence += 1
        worker_id = f"meow-{provider.value}-{self._sequence}"
        queue = self._queues[provider]
