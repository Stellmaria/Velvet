...
class PublicationDraftRepository:
    ...
    async def capture_inbox(self, payload: PublicationInboxPayload) -> None:
        if payload.telegram_file_id is None and not payload.text_content:
            return
        workspace_id = int(getattr(payload, "workspace_id", DEFAULT_WORKSPACE_ID))
    ...
    async def create_draft(
        ...
        *,
        ...
        workspace_id = int(getattr(source, "workspace_id", DEFAULT_WORKSPACE_ID))
        async with self._database.acquire() as connection:
            async with connection.transaction():
                ...
                for item in items:
                    ...
                    item_workspace_id = int(
                        getattr(payload, "workspace_id", DEFAULT_WORKSPACE_ID)
                    )
                ...
                await self._log_event_on_connection(
                    connection,
                    workspace_id=(None if workspace_id == DEFAULT_WORKSPACE_ID else workspace_id),
        ...
        async with self._database.acquire() as connection:
            async with connection.transaction():
                if workspace_id == DEFAULT_WORKSPACE_ID:
                    ...
                    missing = "Черновик не найден в выбранном пространстве."
                if result == "UPDATE 0":
