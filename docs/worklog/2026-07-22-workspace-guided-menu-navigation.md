...
- необязательные назначения получили отдельные кнопки справки;
...
- кнопки назад напрямую перерисовывают карточку, список, структуру или родительское меню без промежуточных старых экранов.
...
Контрактные тесты, inventories и синтаксис обновлены; запущен финальный GitHub Actions CI на очищенной ветке.

### PR и commit

PR `#296`; финальный commit будет указан после merge.
...
Слить PR после зелёного CI и выполнить Supervisor Update.
