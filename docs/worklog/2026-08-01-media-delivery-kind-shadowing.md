...
Hotfix устраняет подтверждённый programming error в repository, но не заменяет production smoke после deployment. Старая успешная задача восстановится только при наличии provider task ID и доступного результата у провайдера. Новая генерация и повторное списание recovery не выполняет.
...
- PR: `#527 Исправить запись durable media delivery`;
- production fix commit: `00bc09776fd62d95dc8893a96663b8cc9eab1808`;
- regression tests commit: `43cb3596fcb52dad7b6f0b5417d54b31ab659b60`.
...
После зелёного CI слить PR #527, обновить production `main`, дождаться одного recovery cycle и проверить счётчики инцидентов #462–#464 и состояние сохранённой задачи без повторной генерации.
