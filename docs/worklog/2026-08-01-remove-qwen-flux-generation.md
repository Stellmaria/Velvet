...
Успешно выполнены architecture inventory check, `compileall` и 10 целевых регрессионных тестов. Отдельный type-check run также завершился успешно. После deployment нужен production smoke: открыть Ауф, проверить корневой экран, список фото-моделей и одну генерацию на оставшейся модели.
