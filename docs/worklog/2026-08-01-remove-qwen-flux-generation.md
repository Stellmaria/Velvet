...
Успешно выполнены architecture inventory check, `compileall` и 10 целевых регрессионных тестов. Канонический UI-тест обновлён с устаревших `Создать` / `Оживить` на `Фото` / `Видео`, а архитектурный inventory пересобран с проверяемой baseline-меткой. Полный CI повторно запускается на окончательном head. После deployment нужен production smoke: открыть Ауф, проверить корневой экран, список фото-моделей и одну генерацию на оставшейся модели.
