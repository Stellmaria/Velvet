...
AST-сканирование production package `velvet_bot/` обнаружило:

- 130 внешних обращений к `Database._require_pool()`;
- 35 production-файлов;
...
| Domain repositories | 12 | первый приоритет, чистая замена boundary при сохранении SQL |
...
1. **Фаза 18N:** `ArchivePreviewRepository`, 2 connection points.
2. `PublicationValidationRepository`, 2 connection points.
3. `PublicationDraftRepository`, 8 connection points.
4. `SystemRepository`, 2 connection points.
5. AI quality, media AI, error center, calibration и report repositories отдельными срезами.
