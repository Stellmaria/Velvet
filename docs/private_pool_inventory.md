...
Последний завершённый срез: Фаза 18S, `PaletteCompositionReportRepository`.

## Результат

AST-сканирование production package `velvet_bot/` фиксирует:

- 114 внешних обращений к `Database._require_pool()`;
- 29 production-файлов;
...
| Repository-классы внутри крупных модулей | 28 | переводить по одному repository с runtime-тестами |
...
1. **Фаза 18T:** `VelvetFormattingReportRepository`, 1 connection point.
2. AI quality, media AI, error center и calibration repositories отдельными срезами.
