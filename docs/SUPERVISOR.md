...
```text
Кнопочное меню владельца в Telegram
        ↓
Velvet Bot
...
## Кнопочное управление Telegram

Обычный вход владельца:

```text
/start
```

Также сохранён короткий резервный вход:

```text
/menu
```

После этого открывается единый центр управления:

- архив персонажей;
- управление персонажами;
- Supervisor и Codex;
- состояние системы и воркеров;
- контроль качества;
- резервные копии;
- аналитика;
- публикации.
...
- PID;
- состояние автоперезапуска;
- crash-loop счётчик;
- подтверждённый перезапуск;
- переход к журналу.

### Git

- текущая ветка и commit;
- состояние рабочего дерева;
- последняя операция;
- безопасное обновление `main`;
- откат последнего развёртывания.

Кнопка обновления выполняет:
...
Можно выбрать последние 50, 150, 500 или 2000 строк. Полный выбранный журнал
...
- открытие задачи по кнопке;
- поиск по ID;
- создание новой задачи ответом на форму без slash-команды;
- обновление статуса;
- применение готового commit;
- отклонение worktree;
- отдельный push после применения.

Формы ввода распознаются по маркеру в сообщении `ForceReply`, поэтому не требуют
хранить временное состояние в памяти Telegram-бота и продолжают работать после
перестроения меню.
...
последние строки журнала в служебный чат. Ручная кнопка перезапуска очищает
счётчик.
