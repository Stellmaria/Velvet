...
Видимая кнопка называется `Ollama: восстановить набор моделей`. Внутренний allowlist-key `ollama-repair-qwen3-vl-4b` сохранён для обратной совместимости, но действие `repair` восстанавливает весь набор:
...
- `Ollama: состояние набора моделей` — показывает конфигурацию, хранилище, распознанные manifests, API-модели и состояние каждой целевой модели;
- `Ollama: запустить локальный сервер` — запускает `ollama serve` без shell;
- `Ollama: настроить набор моделей` — записывает конфигурацию полного набора;
- `Ollama: установить набор моделей` — подключает E и устанавливает только отсутствующие модели;
- `Ollama: проверить набор моделей` — проверяет capabilities всего набора;
- `Ollama: восстановить набор моделей` — выполняет полный recovery.

## Удалённая последовательность

1. Слить обновление и выполнить `Обновить + рестарт`.
2. Открыть `Безопасная консоль`.
3. Выполнить `Ollama: состояние набора моделей`.
4. Выполнить `Ollama: восстановить набор моделей` и подтвердить точную команду.
...
7. Повторно выполнить `Ollama: состояние набора моделей`.
