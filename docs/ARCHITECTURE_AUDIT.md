...
| Роль | Доступ |
|---|---|
| Публичный пользователь | `/start`, `/archive`, `/gallery`; выбор категорий, вселенных, историй и персонажей; открытие, листание и закрытие материалов |
| Модератор `8179531132` | публичный просмотр; `/characters`, `/prompt`, `/setprompt`; разрешённые действия карточек, историй и архивных медиа; скачивание файла из публичного viewer |
| Владелец | все остальные команды и кнопки, включая публикации, аналитику, backup, Supervisor, Git и Codex |

Публичный архив работает только на чтение. Кнопки лайка и подписки удалены, а
`pub:like`, `pub:sub`, `pub:download` и любые неизвестные `pub:*` не считаются
публичными действиями. Скачивание разрешено только указанному модератору.
...
- read-only состав публичной клавиатуры и callback allowlist;
- запрет автоматического наследования неизвестных moderator actions;
