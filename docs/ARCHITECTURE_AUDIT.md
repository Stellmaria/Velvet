...
отдельные boundary services:

- проверка доступа к теме Telegram;
- загрузка экспортного документа;
- создание публикационного черновика;
- сохранение обычного медиа и медиа со спойлером.
...
Кнопочные формы и резервные slash-маршруты используют общие use cases для
персонажей, классификации, историй, алиасов, референсов, аналитики, импорта и
сохранения медиа.
...
- разбор многочастных имён, `off`, индексов референсов и завершение сессий без Telegram.

## Архитектурный долг

### P1: разнести крупные Telegram-контроллеры

После фазы 9 первым кандидатом становится `handlers/supervisor_control.py`.
Далее следует физически разделить рендер и маршрутизацию самого
`handlers/owner_actions.py`, который уже не содержит бизнес-решений, но остаётся
крупным UI-контроллером. Затем идут:
...
Owner action controller очищен. Старые зависимости всё ещё встречаются в UI,
например между picker-контроллерами персонажей, историй и отображения медиа.
Их следует заменять общими render helpers или application coordinators по одному,
не смешивая с изменением поведения.
