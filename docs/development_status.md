...
- корневой Router должен подключать крупные доменные bundles вместо десятков отдельных handlers;
...
Остаётся постепенно переносить активные Telegram-контроллеры из `velvet_bot/handlers` в `velvet_bot/presentation/telegram/routers` с временными re-export фасадами. Перенос выполняется доменными срезами и не меняет callback prefixes или поведение.

### P3D. Compatibility

Активные compatibility-компоненты централизуются и инвентаризируются. Каждый компонент удаляется только после переноса его поведения в канонический module/service. Неиспользуемые мосты удаляются сразу с regression-тестом.
