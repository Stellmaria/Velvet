...
Production legacy-consumer inventory закрыт: 0 файлов, 0 references и 0 legacy modules. Все рабочие imports используют canonical presentation modules или публичные contracts/parsers. Новые imports `velvet_bot.handlers.*` блокируются inventory contract; 68 aliases остаются только для совместимости тестов и внешних import paths.
