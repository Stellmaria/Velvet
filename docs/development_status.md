...
Production legacy-consumer inventory закрыт: 0 файлов, 0 references и 0 legacy modules. Все рабочие imports используют canonical presentation modules или публичные contracts/parsers. Первый compatibility-batch удалил 22 archive/reference aliases, затем удалены два aliases с нулевым repository reference count (`ai_jobs`, `quality_calibration`). Остаются 44 aliases, и все они пока имеют repository references; references на уже удалённые aliases блокируются CI.
