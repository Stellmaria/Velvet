...
Статус: срезы 18A–18U реализованы, работа продолжается.
...
- provider/model filters и calibration limit clamp;
- outcome sections, count query, page-size/page clamp и offset;
- file-name fallback, JSON decode и mapping calibration cases;
- формулы `build_calibration_profile()` и recommended decision.
...
- явные domain/infrastructure, одиночные report и calibration repositories из baseline закрыты;
- baseline контролируется AST-тестом и SHA-256 identity методов;
- новые неучтённые обращения блокируются CI.

Следующий срез: **18V, repository-контур `ai_quality.py`**, 8 connection points. Ожидаемый baseline: 102 обращения в 26 файлах.
...
1. Фаза 18V: repository-контур `ai_quality.py`.
