...
Статус: срезы 18A–18M реализованы; погашение зафиксированного baseline продолжается.
...
- четырнадцать завершённых repositories защищены source/runtime regression-тестами от возврата приватного pool access;
- AST-инвентаризация сканирует прямые обращения и динамический `getattr(..., "_require_pool")`;
- зафиксирован baseline: 130 внешних обращений в 35 production-файлах;
...
Следующий срез: Фаза 18N, `ArchivePreviewRepository`; после него baseline должен уменьшиться с 130 до 128 обращений.
...
1. Выполнить Фазу 18N: перевести `ArchivePreviewRepository` и уменьшить baseline до 128 обращений.
...
4. Уменьшать широкие `except Exception` внутри бизнес-логики.
5. Добавить автоматическую зашифрованную репликацию backup во внешнее хранилище.
6. Подготовить отдельную staging-конфигурацию и отдельного Telegram-бота.
7. Добавить метрики времени AI-задач и стоимости внешнего provider при его подключении.
8. Вынести статические тексты и клавиатуры крупных presentation-фасадов только там, где это реально уменьшает связанность.
