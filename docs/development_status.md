...
Legacy-consumer inventory фиксирует 19 production-файлов, 28 references и 17 старых handler modules после очистки `characters/uncategorized.py`, `stories/management.py` и `stories/multi_story_kr.py`. Новые imports блокируются inventory contract; aliases удаляются небольшими группами после переноса consumers на публичные contracts.
