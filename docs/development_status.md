...
- регрессионный тест запрещает возврат приватного pool access во всех девяти завершённых repositories;
- runtime-тесты проверяют public acquire и транзакционные контракты critical workflows;
- отдельный AST-контракт запрещает аукционные импорты, классы и идентификаторы в production package.

Следующий срез: `DiscussionInsightRepository`.
...
1. Продолжить Фазу 18 с `DiscussionInsightRepository`; characters, stories, archive, public archive, references, media quality, publication, discussions и discussion ingest завершены.
