...
Все активные Telegram-контроллеры находятся в `velvet_bot/presentation/telegram/routers`. Каталог `velvet_bot/handlers` содержит 46 временных re-export/module aliases и 0 implementations.

### P3D. Compatibility

Статус: выполняется.

Production legacy-consumer inventory закрыт: 0 файлов, 0 references и 0 legacy modules. Все рабочие imports используют canonical presentation modules или публичные contracts/parsers. После удаления archive/reference и двух zero-reference aliases character/story compatibility-тесты переведены на canonical modules и удалены ещё 9 aliases. Остаются 35 aliases, и все они пока имеют repository references; references на уже удалённые aliases блокируются CI.
