...
Статус: срезы 18A–18AA реализованы, baseline продолжает уменьшаться.
...
- repository-классы внутри крупных модулей и runtime backup overrides полностью удалены из baseline;
- новые или изменённые private pool access блокируются CI;
- SQL, claim locking, lifecycle, schema-compatible pagination, owner decisions, error acknowledgment/digest, AI response-version и backup schedule semantics завершённых срезов сохранены.

Следующий срез: **18AB, базовый backup service**, 15 connection points.
...
1. Фаза 18AB: backup service, baseline 82 → 67.
2. Telegram import persistence отдельным срезом.
