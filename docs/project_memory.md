...
Все активные Telegram controllers перенесены в `velvet_bot/presentation/telegram/routers/<domain>/`. В `velvet_bot/handlers` остаются 68 временных module aliases и 0 реализаций. Callback prefixes, команды и use cases при переносе не менялись.

## P3D. Compatibility retirement

Статус: выполняется.

Активные adapters находятся в одном pre-import/post-import реестре. Отдельный legacy-consumer inventory фиксирует 19 production-файлов, 28 references и 17 старых handler modules после очистки первых character/story controllers, включая multi-story KR. Каждый alias удаляется только после переноса consumers на канонический service/view/repository и регрессионной проверки.
