...
# Линия A. Основное развитие Velvet Archive
...
# Линия D. Организация структуры P3
...
- root router подключает крупные доменные bundles;
...
Активные handlers постепенно переносятся в `velvet_bot/presentation/telegram/routers/<domain>/`. Старые пути временно становятся тонкими re-export фасадами. За один срез переносится один связный домен без изменения callback prefixes, команд и use cases.
...
Статус: открыт, один неиспользуемый discussion bridge удалён.

Активные adapters должны находиться в одном реестре и иметь явную стадию `pre-import` или `post-import`. Каждый adapter удаляется только после переноса поведения в канонический service/view/repository.
...
# Открытые эксплуатационные обязательства
