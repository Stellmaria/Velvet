...
Активные adapters находятся в одном pre-import/post-import реестре. Production legacy-consumer inventory закрыт: 0 production-файлов, 0 references и 0 старых handler modules. Оставшиеся 68 handler aliases используются только как внешний/test compatibility-слой и удаляются после миграции соответствующих compatibility-тестов и публичных import contracts.
