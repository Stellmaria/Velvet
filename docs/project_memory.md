...
Статус: срезы 18A–18T реализованы, baseline продолжает уменьшаться.
...
- явные domain и infrastructure repositories, а также одиночные report repositories из baseline закрыты;
- новые или изменённые private pool access блокируются CI;
- SQL, параметры и mapping завершённых repositories сохранены.

Следующий срез: **18U, `QualityCalibrationRepository`**, 3 connection points.
...
1. Фаза 18U: `QualityCalibrationRepository`, baseline 113 → 110.
2. Переводить AI quality, media AI и error center repositories отдельными срезами.
