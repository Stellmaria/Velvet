...
Активные adapters находятся в одном pre-import/post-import реестре. Production legacy-consumer inventory закрыт: 0 production-файлов, 0 references и 0 старых handler modules. После archive/reference и zero-reference срезов character/story compatibility-тесты переведены на canonical modules и удалены ещё 9 aliases; остаются 35 aliases. Alias-consumer inventory подтверждает, что все 35 оставшихся aliases пока имеют repository references. Удаление продолжается связанными группами после миграции тестов на canonical modules.
