...
Статус: срезы 18A–18U реализованы, baseline продолжает уменьшаться.
...
- явные domain и infrastructure repositories, одиночные report repositories и calibration repository из baseline закрыты;
- новые или изменённые private pool access блокируются CI;
- SQL, фильтры, пагинация и mapping завершённых repositories сохранены.

Следующий срез: **18V, repository-контур `ai_quality.py`**, 8 connection points.
...
1. Фаза 18V: repository-контур `ai_quality.py`, baseline 110 → 102.
2. Переводить media AI и error center repositories отдельными срезами.
