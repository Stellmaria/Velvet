...
Статус: срезы 18A–18V реализованы, baseline продолжает уменьшаться.
...
- SQL, claim locking, lifecycle, schema-compatible pagination и owner decisions завершённых repositories сохранены.

Следующий срез: **18W, repository-контур `ai_vision.py`**, 4 connection points.
...
# Линия D. Heavy Runtime orchestration

Цель: оставить один постоянно доступный Telegram-бот, но управлять Qwen/Ollama, Krita, тяжёлым импортом, агрегацией и backup как запускаемыми по необходимости runtime-компонентами.

Статус:

- HR-1: инвентаризация процессов, workers, feature flags и точек запуска тяжёлых сервисов — подготовлена в отдельной docs-only ветке;
- полный source inventory: `docs/heavy_runtime_inventory.md`;
- production lifecycle пока не менялся;
- Фаза 18W остаётся независимым repository-срезом.

Последовательность:

1. HR-2: единые feature flags и runtime timeouts без изменения поведения.
2. HR-3: `ResourceManager` foundation, приоритеты, состояния и unit-тесты.
3. HR-4: `LocalModelRuntime` для Ollama.
4. HR-5: demand-aware Qwen scheduling и idle unload.
5. HR-6: один owner-only RP runtime без отдельного Telegram-бота.
6. HR-7: adapter существующего Krita process manager.
7. HR-8: лёгкие analytics events и тяжёлая агрегация.
8. HR-9: checkpoint import и event-driven backup.
9. HR-10: Supervisor runtime dashboard и Windows smoke-test.

Архитектурные решения:

- не выполнять горячую выгрузку aiogram-router;
- не создавать микросервис на каждый handler/repository;
- Supervisor остаётся process-control слоем;
- новый `ResourceManager` живёт в приложении и публикует snapshot Supervisor;
- существующие PostgreSQL AI queues и Krita process safety переиспользуются, а не дублируются.
...
1. Фаза 18W: repository-контур `ai_vision.py`, baseline 100 → 96.
2. HR-2: единые feature flags и runtime timeouts без изменения текущего scheduling.
3. Переводить error center и Ollama/resilient AI repositories отдельными срезами.
4. Перенести семь прямых DB access из handlers в application/domain services.
5. Проверить все долгие callback-сценарии и ранний acknowledgment.
6. Сократить широкие `except Exception` в бизнес-логике.
7. Подготовить staging-бота и staging-базу.
8. Провести независимый backup/restore drill.
9. Добавить зашифрованную внешнюю репликацию backup.
10. Добавить метрики длительности и ошибок AI-задач.
