...
Статус: срезы 18A–18Y реализованы, baseline продолжает уменьшаться.
...
- явные domain/infrastructure, одиночные report, calibration, AI quality, Media AI, error incident и reliable Ollama repositories вместе с активным compatibility facade из baseline закрыты;
- новые или изменённые private pool access блокируются CI;
- SQL, claim locking, lifecycle, schema-compatible pagination, owner decisions, error acknowledgment/digest и Ollama response-version semantics завершённых repositories сохранены.

Следующий срез: **18Z, resilient AI vision repository в `resilient_ai_vision.py`**, 2 connection points.
...
1. Фаза 18Z: resilient AI vision repository, baseline 86 → 84.
2. Перенести семь прямых DB access из handlers в application/domain services.
